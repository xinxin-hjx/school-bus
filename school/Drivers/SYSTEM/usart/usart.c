...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
    GPIO_InitTypeDef gpio_init_struct;
		GPIO_InitTypeDef gpio_u2init_struct;
    ...
    else if (huart->Instance == ATK_MW8266D_UART_INTERFACE)                 /* 如果是ATK-MW8266D UART */
    {
		...
		else if (huart->Instance == mp3_usart)                 
    {
			mp3_TX_CLK_ENABLE();                            
        mp3_RX_CLK_ENABLE();                            
        USART_U2_CLK_ENABLE();                                 
        gpio_u2init_struct.Pin = mp3_TX_GPIO_PIN;               /* 串口发送引脚号 */
        gpio_u2init_struct.Mode = GPIO_MODE_AF_PP;                /* 复用推挽输出 */
        gpio_u2init_struct.Pull = GPIO_PULLUP;                    /* 上拉 */
        gpio_u2init_struct.Speed = GPIO_SPEED_FREQ_HIGH;          /* IO速度设置为高速 */
        HAL_GPIO_Init(mp3_TX_GPIO_PROT, &gpio_u2init_struct);
                
        gpio_u2init_struct.Pin = mp3_RX_GPIO_PIN;               /* 串口RX脚 模式设置 */
        gpio_u2init_struct.Mode = GPIO_MODE_AF_INPUT;    
        HAL_GPIO_Init(mp3_RX_GPIO_PROT, &gpio_u2init_struct);   /* 串口RX脚 必须设置成输入模式 */
        
#if USART_EN_RX
        HAL_NVIC_EnableIRQ(USART_U2_IRQn);                      /* 使能USART1中断通道 */
        HAL_NVIC_SetPriority(USART_U2_IRQn, 2, 3);              /* 组2，最低优先级:抢占优先级3，子优先级3 */
#endif        
    }
}
